...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    return MoPDOConv(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False)

class MoPDOConv(nn.Conv2d):
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, padding=0, bias=False, enable_mopdo=False):
        super(MoPDOConv, self).__init__(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias)
        self.enable_mopdo = enable_mopdo
        self.k = kernel_size
        if self.enable_mopdo:
            self.scale_col = nn.Parameter(torch.empty(1, in_planes * kernel_size), requires_grad=True)
            self.scale_line = nn.Parameter(torch.empty(out_planes * kernel_size, 1), requires_grad=True)
            if bias:
                self.shift_bias = nn.Parameter(torch.empty(1, out_planes), requires_grad=True)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = MoPDOConv(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = MoPDOConv(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = MoPDOConv(planes, planes * 4, kernel_size=1, bias=False)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.inplanes = 64
        self.conv1 = MoPDOConv(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                MoPDOConv(self.inplanes, planes * block.expansion, kernel_size=1,
